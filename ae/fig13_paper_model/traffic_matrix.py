...
class TrafficMatrix:
    ...
    def __init__(self, N: int = 256):
        self.N = N
        self.counts = np.zeros((N, N), dtype=np.uint64, order="C")  # Row-major, cache-friendly.
    ...
    def add_many(self, srcs, dsts, values) -> None:
        ...
        np.add.at(flat, idx.astype(np.intp, copy=False), v)  # Integer path, with no precision loss.
    ...
    def topk_pairs(self, k: int = 10) -> list[tuple[int, int, int]]:
        ...
        if k >= flat.size:
            k = flat.size
        # Use argpartition to find the indices of the k largest entries, then sort by value.
    ...
    def save_heatmap(
        ...
        Args:
            path: Output file path, such as .png or .pdf.
            cmap: Color map, such as "viridis", "magma", or "inferno".
            normalize: Whether to normalize by the maximum value to [0,1],
                independently of log_scale.
            log_scale: Whether to use a logarithmic color scale, suitable for long-tailed
                distributions. Zero values appear in the background color.
            vmin/vmax: Manually specified color limits (float), inferred automatically
                when omitted.
        ...
        try:
            import matplotlib.pyplot as plt  # Import lazily to avoid a hard dependency.
            from matplotlib.colors import LogNorm
        except Exception as e:  # noqa: BLE001 - clearly report the required dependency.
            raise ImportError(
        ...
        # Normalize (without affecting vmin/vmax inference for log_scale).
        ...
        if log_scale:
            ...
            if positive.size > 0:
                ...
                # Avoid errors from vmin==vmax or vmin<=0.
                ...
                norm = None  # All 0: a linear color scale suffices.
    ...
    def export_json(self, path: str) -> None:
        ...
        # Select index pairs to export: off-diagonal with traffic > 0.
        ...
        # Build the entry list (convert to Python int for JSON serialization).
        entries = [
            {
                "src": int(s),
                "dst": int(d),
        ...
        # Read the existing JSON to determine the new time_stamp.
        data: list[dict] = []
        last_ts = -1
        if os.path.exists(path):
            try:
                ...
                # If it is not a list or is empty, treat it as a new file.
            except Exception:
                # If parsing fails, reset and write as a new file.
                data = []
                last_ts = -1
        ...
        # Write back by rewriting the entire file for simplicity and reliability.
    ...
    def _compute_parallelism_groups(
        ...
        # Precompute prefix products (for strides) and suffix products (for outer block counts).
        ...
        for level, (gsize, name) in enumerate(zip(dims, names)):
            ...
            stride = prefix[level]              # Stride = product of all preceding dimensions.
            outer_blocks = suffix[level + 1]    # Outer block count = product of all subsequent dimensions.
            if N % gsize != 0:
                raise ValueError(f"N 必须能被 {name} 整除（N={N}, {name}={gsize}）")

            groups: list[list[int]] = []
            # Total group count should be N / gsize = stride * outer_blocks.
    ...
    def visualize_parallelism_groups(
        ...
        # Compute each dimension's stride (prefix product).
        ...
        for i, ((name, size), ax) in enumerate(zip(dims, axes)):
            ...
            for gid, arr in enumerate(g):
                for n in arr:
                    node_colors[n] = gid % 20  # Cycle through 20 colors.
    ...
    def add_intra_group_traffic(
        ...
        *,
        ...
        # For each group: add inc to every element of its submatrix at once, then subtract inc from the diagonal by default.
        for g in groups[key]:
            m = len(g)
            if m == 0:
                continue
            # Submatrix slice (rows and columns both belong to this group).
            idx = np.ix_(g, g)
            self.counts[idx] += inc  # Add inc to every element within the group.
            if not include_self:
                # Subtract inc from the diagonal (add first, then subtract to avoid uint64 underflow).
                self.counts[g, g] -= inc
    ...
    def add_intra_group_traffic_pair(
        ...
        *,
        ...
        d_list: list[int] = []
        for g in groups[key]:
            # Within each group, member src sends to member dst.
            s_list.append(g[src])
            d_list.append(g[dst])
    ...
    def intragroup_traffic_stats(
        ...
        *,
        ...
        # Compute each dimension's stride (inner-to-outer prefix product, excluding itself).
        dim_sizes = [tp, ep, sp, cp, dp, pp]
        strides = []
        s = 1
    ...
    # ======= Matrix addition (supports saturating addition to avoid uint64 overflow) =======
    def _saturating_add_arrays(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        ...
        res = a + b
        overflow = res < a  # uint64 overflow wraps around to a smaller value.
        if np.any(overflow):
            res = res.copy()
    ...
    def add_matrix(self, other: "TrafficMatrix", *, saturating: bool = True, in_place: bool = False) -> "TrafficMatrix":
        ...
        if saturating:
            ...
            if in_place:
                self.counts += other.counts  # May wrap around.
                return self
            else:
                out = TrafficMatrix(self.N)
...
def test_TrafficMatrix_only():
    ...
    # tm.add(1, 2, 1500)             # Add 1500 to 1->2.
    # tm.add(1, 2, 500)              # Add another 500 to 1->2.
    # tm.add_many([0, 5, 5], [3, 7, 7], [100, 200, 300])  # Batch update.

    # Along the SP dimension: add 4KB to each pair within each SP group (size 8, 32 groups total).
    tm.add_intra_group_traffic("dp", 1024, tp=8, ep=1, sp=4, cp=1, dp=4, pp=2)

    # Different amounts can also be added to multiple dimensions at once, for example:
    # tm.add_intra_group_traffic_bulk(
    #     {"tp": 8 * 1024, "sp": 4 * 1024, "dp": 2 * 1024, "pp": 1 * 1024},  # +4KB per pair in SP groups, +2KB per pair in DP groups, +2KB per pair in PP groups.
    #     tp=8, ep=1, sp=8, cp=1, dp=2, pp=2
    # )
    ...
    # -> Includes fields such as total_intragroup / avg_per_group / avg_per_node_out.

    # 2) Add two matrices (return a new object using saturating addition).
    tm_sum = tm + tm2
...
def test_TrafficMatrix_ep_all_to_all():
    ...
    "ep",
    triples=[
        [2048, 0, 2],  # Intragroup 0->2 (global 16->20 for the 1st group).
        [1024, 1, 3],
    ],
    group_index=1,
...
def test_traffic_matrix_intra_group_pair_unit():
    ...
    # Validate the expected edges.
    # for (s, d), v in expected.items():
    #     assert int(tm.counts[s, d]) == v, f"{s}->{d} should be {v}, got {int(tm.counts[s, d])}"
    # # Validate the total.
    # total, _, _ = tm.totals()
    # assert total == sum(expected.values()), f"Total should be {sum(expected.values())}, got {total}"
