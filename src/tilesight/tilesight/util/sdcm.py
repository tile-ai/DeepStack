...
def _norm_cdf_approx_3(x, a2):
    a1, a3 = 0.4361836, 0.937298
    k = 1.0 / (1.0 + 0.33267 * abs(x))
...
def norm_cdf_approx_3(x):
    # The quadratic coefficient is negative.  The released positive sign
    # made Phi(0) approximately 0.404 and introduced a discontinuity at zero.
    return _norm_cdf_approx_3(x, -0.1201676)
...
def sdcm(D, A, B):
    ...
    if D <= A - 1:
        ...
        if get_model_version() == "paper_legacy":
            prob = _norm_cdf_approx_3(norm_input, 0.1201676)
        else:
