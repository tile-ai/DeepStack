...
def _resource_values(ret):
    """Return legacy resources plus optional useful/padded tensor work."""
    values = tuple(ret)
    if len(values) < 9:
        raise ValueError(
            f"resource result must contain at least 9 values, got {len(values)}"
        )
    useful_tensor_ops = float(values[9]) if len(values) >= 11 else float(values[6])
    padding_tensor_ops = float(values[10]) if len(values) >= 11 else 0.0
    return (*values[:9], useful_tensor_ops, padding_tensor_ops)


def hete_post_process_tensor_core_op(ret, arch:Arch , data_bytes):
    ddr_io, l2_hit_rate, l2_io, smem_footprint, smem_l1_io, reg_footprint, compute_flops, ddr_read_io, l2_read_io, useful_tensor_ops, padding_tensor_ops = _resource_values(ret)

    ddr_time = ddr_io/arch.ddr_bandwidth
    l2_time = l2_io/arch.l2_bandwidth
    smem_l1_time = smem_l1_io/arch.smem_bandwidth
    if data_bytes==4:
        tensor_peak_flops = arch.fp32_tensor_flops
    elif  data_bytes==2:
        tensor_peak_flops = arch.fp16_tensor_flops
    elif data_bytes==1:
        # if (fp8_tensor_flops exist)
        if hasattr(arch, 'fp8_tensor_flops'):
            tensor_peak_flops = arch.fp8_tensor_flops
        elif hasattr(arch, 'int8_tensor_flops'):
            tensor_peak_flops = arch.int8_tensor_flops
        else:
            raise ValueError("fp8_tensor_flops or int8_tensor_flops not found")
    else:
        raise ValueError(f"unsupported tensor-core data width: {data_bytes}")
    compute_time = compute_flops/tensor_peak_flops
    ...
    energy_scale = arch.fp16_tensor_flops / tensor_peak_flops
    result = bound_time, ddr_util, l2_hit_rate, l2_util, smem_footprint, smem_l1_util, reg_footprint, ddr_read_io, l2_read_io, tensor_util, cuda_util, sfu_util, useful_tensor_ops * energy_scale, padding_tensor_ops * energy_scale
    return result
...
def hete_post_process_cuda_core_op(ret, arch:Arch, data_bytes):
    ...
    # result = bound_time, 0, ddr_util, l2_hit_rate, l2_util, smem_footprint, smem_l1_util, reg_footprint, compute_util, ddr_read_io, l2_read_io
    result = bound_time, ddr_util, l2_hit_rate, l2_util, smem_footprint, smem_l1_util, reg_footprint, ddr_read_io, l2_read_io, tensor_util, cuda_util, sfu_util, 0.0, 0.0
    return result
