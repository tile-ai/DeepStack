...
def calculate_matmul_triton_swizzle_resource_utilization(op_shape,tb_shape,wp_shape, stage_num,arch,mem_levels,group_m=8,batch=1):
    m,n,k=op_shape
    ...
    DDR_non_ideal_para = 1.0 #already reflected in adjusted ddr bandwidth
    REG_spill_para=1.1 # just like the last one, something we don't know clearly about ncu

    compute_flops=m*n*k*2
    l2_hit_rate=L2_hit_rate_flow_sim_reuse_distance_triton_swizzle(m, n, k, tb_m, tb_n, tb_k, arch.l2_capacity, arch.sm_count, mem_levels, group_m)
    # l2_hit_rate=L2_hit_rate_flow_sim_reuse_distance_triton_swizzle(m, n, k, tb_m, tb_n, tb_k, arch.l2_capacity, arch.sm_count * 2, mem_levels, group_m)
    # l2_hit_rate=0.2436
