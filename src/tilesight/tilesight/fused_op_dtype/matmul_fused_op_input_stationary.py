...
def calculate_matmul_input_stationary_resource_utilization(op_shape,tb_shape,wp_shape, stage_num,arch,mem_levels,row_panel=108,batch=1):
    m,n,k=op_shape
    tb_m,tb_n,tb_k=tb_shape
    wp_m,wp_n,wp_k=wp_shape

    assert k==tb_k, 'K must equal threadblock_k to enable efficient input stationary gemm'
