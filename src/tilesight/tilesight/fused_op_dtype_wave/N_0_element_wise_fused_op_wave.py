# Filename: N_0_element_wise_op.py
...
def calculate_N_0_elementwise_resource_utilization(op_shape,tb_shape,dim_threads, arch,mem_levels,num_ops=1):
    ...
    #     mem_levels = {
    #     'in1': '[1,1,1...,4]', for ddr
    #     'in2': '[0,1,1,2]', for smem
    #     'out1': [0,0,1,4],  for reg
    # }

    in1_level = mem_levels['in1']
    out1_level = mem_levels['out1']

    l2_read_io=np.prod(op_shape)*in1_level[0]*in1_level[-1]
    ...
    smem_l1_io=(np.prod(op_shape)*in1_level[1])*in1_level[-1]
    ...
    reg_footprint=math.ceil(np.prod(thread_tiles)*(in1_level[1]*in1_level[-1]/4+1*out1_level[1]/4)*REG_spill_para)
