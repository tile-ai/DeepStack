...
# def fa_prefill_wrapper(shard_bs, shard_num_head, shard_seq_q, shard_seq_kv, head_dim, atten_parallel:ParallelScheme, atten_bytes:OpBytes,granularity:Modeling_Granularity, single_chip:Arch):
def fa_prefill_wrapper(bs:int, seq:int, hidden:int, num_head:int, num_kv_head:int, head_dim:int, parallel:ParallelScheme, atten_parallel:ParallelScheme,
    ...
    if mode == "coarse":
        if tune_flag==False:
            ...
            grids, time = model_fa_prefill_coarse_four_stage_pessimistic(bs, seq, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, atten_bytes, granularity, single_chip, noc_hierarchy, tiling_config, stats=stats)
            ...
            for config in filtered_configs:
                tb_m, tb_n, tb_k, wp_m, wp_n, wp_k, stages = config
                tiling_config =(tb_m, tb_n, tb_k, wp_m, wp_n, wp_k, stages)
                grids, time = model_fa_prefill_coarse_four_stage_pessimistic(bs, seq, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, atten_bytes, granularity, single_chip, noc_hierarchy, tiling_config)
                if time < min_time:
                    min_time = time
                    best_grids = grids
                    best_config = tiling_config
            if stats is not None and best_config is not None:
                model_fa_prefill_coarse_four_stage_pessimistic(bs, seq, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, atten_bytes, granularity, single_chip, noc_hierarchy, best_config, stats=stats)
            return best_grids, min_time
            # raise NotImplementedError("Auto tune is not implemented for fa prefill")


def model_fa_prefill_coarse_four_stage_pessimistic(bs:int, seq:int, hidden:int, num_head:int, num_kv_head:int, head_dim:int, parallel:ParallelScheme, atten_parallel:ParallelScheme,
    ...
    def stage2():
        ...
        compute_flops+=np.prod(grids)*head_dim*shard_seq_kv*2
        ...
        if single_chip.support_wgmma:
            smem_l1_io+=np.prod(grids)*(tb_m*(head_dim)*shard_seq_kv/tb_n + head_dim*shard_seq_kv*(1+tb_m/wp_m))*in_bytes
        else:
