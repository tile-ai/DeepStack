"""
lat_bw_dse_decode.py — NoC latency vs bandwidth DSE for decode (DeepSeek R1 only)
...
def lat_bw_decode_tasks():
    # tasks = lat_bw_decode_tasks_full()
    tasks = lat_bw_decode_tasks_short_kv_len()
    return tasks

def lat_bw_decode_tasks_full():
    tasks = []
    model = DeepSeekV3()
    PARALLEL_DECODING = 1
...
def lat_bw_decode_tasks_short_kv_len():
    tasks = []
    model = DeepSeekV3()
    PARALLEL_DECODING = 1
...
def dse_1(run_dir, num_workers, enable_proc_log=False):
    ...
    task_combinations = lat_bw_decode_tasks()
        ...
        for task_idx, task_combination in enumerate(task_combinations):
            ...
            if rows:
                ...
                with open(csv_path, mode="a", newline="") as f:
                    ...
                    for r in rows:
                        # r layout:
                        #  0: baseline_name, 1: lat_mult, 2: bw_mult,
