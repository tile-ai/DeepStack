...
def task_decoding_combination():
    ...
    # for model_arch in [ LLM_MOE_SPARSE_TO_DENSE_128()]:
        for BS in BS_LIST:
...
def _init_worker(npz_trace_file: str):
    ...
    # Extract the number after _a and before /.
    # Locate the string after _a and before the final /.
    import re
...
def _compute_time_row(args):
    ...
    # Return lightweight results (strings/numbers/small objects).
    # return [
    #     arch.__class__.__name__, noc.name, model_arch.__class__.__name__,
    #     BS, minibatch, 1, kv_len,
    #     str(parallel_scheme),                  # Returning a string is more robust.
...
def dse_1(run_dir, num_workers):
    ...
    for task_combination in task_combinations:
        ...
        if model_arch.__class__.__name__ == "DeepSeekV3":
            ...
            # npz_trace_file = str(project_root / "data" / "aime_qwen_235b" / "qwen3_moe_activations_batch0.npz")
            raise ValueError(f"Unsupported model architecture: {model_arch.__class__.__name__}")
