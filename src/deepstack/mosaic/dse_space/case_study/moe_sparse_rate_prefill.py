...
from mosaic.dse_space.case_study.llm_arch_moe_sparse_to_dense import LLM_MOE_SPARSE_TO_DENSE_1, LLM_MOE_SPARSE_TO_DENSE_8, LLM_MOE_SPARSE_TO_DENSE_32, LLM_MOE_SPARSE_TO_DENSE_128
...
def task_decoding_combination():
    ...
    for model_arch in [LLM_MOE_SPARSE_TO_DENSE_1(), LLM_MOE_SPARSE_TO_DENSE_8(), LLM_MOE_SPARSE_TO_DENSE_32(), LLM_MOE_SPARSE_TO_DENSE_128()]:
        for BS in BS_LIST:
...
def _init_worker(npz_trace_file: str):
    ...
    import re
    basename = npz_trace_file.split("/")[-2]  # e.g., "qwen_e128_a128"
    match = re.search(r"_a(\d+)", basename)
    if match:
        ...
        elif extracted_str == "8":
            num_activated_experts = 8
        elif extracted_str == "32":
            num_activated_experts = 32
        elif extracted_str == "128":
            num_activated_experts = 128
        else:
            raise ValueError(f"未能成功提取字符串。")
        # print(f"String converted to number: {num_activated_experts}")
    else:
        # print("Failed to extract the string.")
        raise ValueError(f"未能成功提取字符串。")

    _, decode_array = load_npz_routing_keep_shape(npz_trace_file, as_list=False, expected_last_dim=num_activated_experts)
...
def get_max_footprint_prefill(model_arch:LLM_Arch, bs:int, seq:int, cached_kv:int, 
    parallel:ParallelScheme, atten_parallel:ParallelScheme, moe_parallel:ParallelScheme, non_atten_non_moe_parallel:ParallelScheme):
    ...
    if model_arch.moe_arch is not None:
        moe_max_activation, moe_mem_weight = get_moe_footprint_wrapper(model_arch=model_arch, cached_kv=cached_kv, moe_parallel=moe_parallel)
...
def modeling_prefill(model_arch:LLM_Arch, bs:int, seq:int, cached_kv:int, parallel:ParallelScheme, atten_parallel:ParallelScheme, moe_parallel:ParallelScheme, non_atten_non_moe_parallel:ParallelScheme, single_chip:Arch, noc_hierarchy:Hierarchy, granularity:Modeling_Granularity, routing_array: np.ndarray):
    ...
    if model_arch.gqa_arch is not None:
        time_gqa, _ = gqa_prefill_top(bs=bs, seq=seq, hidden=hidden, num_head=model_arch.gqa_arch.num_head, num_kv_head=model_arch.gqa_arch.num_kv_head, head_dim=model_arch.gqa_arch.head_dim, parallel=non_atten_non_moe_parallel, atten_parallel=atten_parallel, next_parallel=non_atten_non_moe_parallel, atten_bytes=model_arch.gqa_arch.atten_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
        time_gqa = time_gqa * shard_layer
    else:
        time_gqa = 0

    if model_arch.mla_arch is not None:
        time_mla = mla_prefill_top(bs=bs, seq=seq, model_arch=model_arch, parallel=non_atten_non_moe_parallel, atten_parallel=atten_parallel, next_parallel=non_atten_non_moe_parallel, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
        time_mla = time_mla * shard_layer
    else:
        time_mla = 0
    ...
    if model_arch.moe_arch is not None:
        time_moe, _ = moe_top(bs=bs, seq=seq, hidden=hidden, moe_down_hidden=model_arch.moe_arch.moe_down_hidden, parallel=moe_parallel, next_parallel=non_atten_non_moe_parallel, expert_bytes=model_arch.moe_arch.expert_bytes, gate_bytes=model_arch.moe_arch.gate_bytes, num_shared_experts=model_arch.moe_arch.num_shared_experts, num_routed_experts=model_arch.moe_arch.num_routed_experts, num_activated_experts=model_arch.moe_arch.num_activated_experts, routing_array=routing_array, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
        shard_moe_layer = math.ceil(model_arch.num_moe_layer/parallel.pp)
        time_moe = time_moe * shard_moe_layer
    else:
        time_moe = 0   

    time_rms_norm, _ = rms_norm_top(bs=bs, seq=seq, hidden=hidden, parallel=non_atten_non_moe_parallel, next_parallel=non_atten_non_moe_parallel, rms_norm_bytes=model_arch.rms_norm_bytes, 
        granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
    time_rms_norm = time_rms_norm * shard_layer * 2

    time_add_residual=add_residual_top(bs=bs, seq=seq, hidden=hidden, parallel=non_atten_non_moe_parallel, next_parallel=non_atten_non_moe_parallel, add_residual_bytes=model_arch.add_residual_bytes, 
        granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
    ...
    return pp_p2p_time, time_gqa, time_mla, time_dense_ffn, time_moe, time_rms_norm, time_add_residual, time_total
...
def _compute_time_row(args):
    ...
    pp_p2p_time, time_gqa, time_mla, time_dense_ffn, time_moe, time_rms_norm, time_add_residual, time_total = modeling_prefill(
    ...
    return [
        ...
        pp_p2p_time, time_gqa, time_mla, time_dense_ffn, time_moe, time_rms_norm, time_add_residual,
        ttft_per_pp, ttft, utps, stps
    ]
...
def dse_1(run_dir, num_workers):
    ...
    task_combinations = task_prefill_combination()
    ...
    for task_combination in task_combinations:
        ...
        if model_arch.__class__.__name__ == "DeepSeekV3":
            ...
            npz_trace_file = str(project_root / "data" / "qwen_e128_a1" / "moe_activations.npz")
        elif model_arch.__class__.__name__ == "LLM_MOE_SPARSE_TO_DENSE_8":
            npz_trace_file = str(project_root / "data" / "qwen_e128_a8" / "moe_activations.npz")
        elif model_arch.__class__.__name__ == "LLM_MOE_SPARSE_TO_DENSE_32":
            npz_trace_file = str(project_root / "data" / "qwen_e128_a32" / "moe_activations.npz")
        elif model_arch.__class__.__name__ == "LLM_MOE_SPARSE_TO_DENSE_128":
            npz_trace_file = str(project_root / "data" / "qwen_e128_a128" / "moe_activations.npz")
        else:
            # won't be used, just for compatibility
            npz_trace_file = npz_trace_file = str(project_root / "data" / "aime_qwen_235b" / "qwen3_moe_activations_batch0.npz")
        ...
        for combo_idx, combination in enumerate(combinations):
            ...
            # Cache footprints to avoid repeated computation; keys are ParallelScheme objects (__hash__ is already implemented).
            ...
            for scheme in filtered_parallel_schemes:
                ...
                #     non_atten_parallel, ep=1, ep1=1, ep2=1, dp=scheme.dp * scheme.ep
                # )
                ...
                if ((max_activation + global_total_weight + global_kv_cache) <= 1.0 * arch.ddr_capacity):
                    ...
                    scheme_footprints[scheme] = (minibatch, max_activation, global_total_weight, global_kv_cache, atten_parallel, moe_parallel, non_atten_non_moe_parallel)
                else:
                    log.info("parallel scheme: %s", scheme)
                    log.info("max_activation: %s GiB, global_total_weight: %s GiB, global_kv_cache: %s GiB",
                             max_activation/(1024**3), global_total_weight/(1024**3), global_kv_cache/(1024**3))
            ...
            for scheme in valid_parallel_schemes:
                minibatch, max_activation, global_total_weight, global_kv_cache, atten_parallel, moe_parallel, non_atten_non_moe_parallel = scheme_footprints[scheme]
                # for kv_len in sampled_kv_len:
                #     tasks.append((
                ...
                tasks.append((
                    ...
                    max_activation, global_total_weight, global_kv_cache,
                    gran_tuple,             # Triple of scalar values.
                    atten_parallel, moe_parallel, non_atten_non_moe_parallel        # Top-level dataclass, picklable.
                ))
...
if __name__ == "__main__":
    ...
    run_dir = os.path.join("./moe_sparse_rate_prefill_runs", run_tag)
