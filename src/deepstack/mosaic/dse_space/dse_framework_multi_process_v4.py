...
from mosaic.dse_space.task_combination import dpsk_decode_task_combination, qwen3_235b_a22b_decode_task_combination, llama3_70b_decode_task_combination, llama3_405b_decode_task_combination
...
def modeling_decode(model_arch:LLM_Arch, bs:int, seq:int, cached_kv_list:list[int], moe_parallel:ParallelScheme, non_moe_parallel:ParallelScheme, single_chip:Arch, noc_hierarchy:Hierarchy, granularity:Modeling_Granularity, routing_array: np.ndarray):
    ...
    debug_tp_ep_parallel_scheme = dataclasses.replace(
        moe_parallel, tp = 1, ep = moe_parallel.ep * moe_parallel.tp, ep1 = moe_parallel.ep * moe_parallel.tp, ep2 = 1
                )
    ...
    if model_arch.gqa_arch is not None:
        time_gqa_list, _ = gqa_decode_kv_list_top(bs=bs, seq=seq, cached_kv_list=cached_kv_list, hidden=hidden, num_head=model_arch.gqa_arch.num_head, num_kv_head=model_arch.gqa_arch.num_kv_head, head_dim=model_arch.gqa_arch.head_dim, parallel=parallel, atten_parallel=parallel, next_parallel=parallel, atten_bytes=model_arch.gqa_arch.atten_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
        time_gqa_list = [x * shard_layer for x in time_gqa_list]
    else:
        time_gqa_list = [0] * len(cached_kv_list)

    if model_arch.mla_arch is not None:
        time_mla_list, _ = mla_decode_kv_list_top(bs=bs, seq=seq, cached_kv_list=cached_kv_list, model_arch=model_arch, parallel=parallel, atten_parallel=parallel, next_parallel=parallel, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
    ...
    time_add_residual, _ = add_residual_top(bs=bs, seq=seq, hidden=hidden, parallel=parallel, next_parallel=parallel, add_residual_bytes=model_arch.add_residual_bytes,
        granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
