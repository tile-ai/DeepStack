"""
noc_bw_dse_prefill.py — NoC per-layer BW sensitivity DSE for prefill (DeepSeek R1 only)

Outer loop: generate_noc_bw_configs()     (baseline, scaled_layer, bw_mult)
Inner loop: prefill tasks                 (model, BS, SEQ, SEQ, SEQ)

Reuses modeling_prefill / get_max_footprint_prefill from the original framework.
"""
...
def noc_bw_prefill_tasks():
    # tasks = noc_bw_prefill_tasks_full()
    tasks = noc_bw_prefill_tasks_short_kv_len()
    return tasks

def noc_bw_prefill_tasks_full():
    tasks = []
    model = DeepSeekV3()
...
def noc_bw_prefill_tasks_short_kv_len():
    tasks = []
    model = DeepSeekV3()
    for BS in [1, 4, 16, 64, 256, 1024]:
...
def _compute_time_row(args):
    ...
    _dump_stats_bundle(
        ...
        metadata={
            "baseline_noc": baseline_name, "scaled_layer": scaled_layer, "bw_multiplier": bw_mult,
...
def dse_1(run_dir, num_workers, enable_proc_log=False):
    ...
    task_combinations = noc_bw_prefill_tasks()
    noc_configs = generate_noc_bw_configs()

    csv_path = os.path.join(run_dir, "noc_bw_prefill_result.csv")
    invalid_csv_path = os.path.join(run_dir, "noc_bw_prefill_invalid.csv")
