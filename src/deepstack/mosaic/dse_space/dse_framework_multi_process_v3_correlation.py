...
def _compute_time_row(args):
    (
        world_size,              # int -> arch_noc_combinations1()[combo_idx]
    ...
    from mosaic.noc.noc_gen import noc_gen_torus_mesh_switch
    arch = stacked_gpu_base()
    noc = noc_gen_torus_mesh_switch(world_size)
...
def dse_correlation(run_dir, num_workers):
    ...
    task_combinations = [
        ...
        [Qwen3_235b_a22b(), 128, 1024, 8192,1],
        # [Llama3_70b(), 16, 1024, 64*1024, 1]
    ]
    from mosaic.dse_space import task_combination_2
    ...
    from mosaic.dse_space.parallel_schemes import gen_correlation_parallel_schemes
    parallel_schemes = gen_correlation_parallel_schemes(4096)
    ...
    for task_combination in task_combinations:
        ...
        for scheme in filtered_parallel_schemes:
            ...
            from mosaic.noc.noc_gen import noc_gen_torus_mesh_switch
            arch = stacked_gpu_base()
            noc = noc_gen_torus_mesh_switch(scheme.world_size())
        ...
        for scheme in valid_parallel_schemes:
            ...
            #     ))
            tasks.append((
                scheme.world_size(),              # arch/noc index; rebuild in the subprocess.
                model_key,              # Model name; rebuild in the subprocess.
...
if __name__ == "__main__":
    ...
    dse_correlation(run_dir, num_workers)
    print("dse_correlation finished")
    end_time = time.time()
    print("dse_correlation time: %s seconds" % (end_time - start_time))