...
class ModelPerfStats:
    ...
    def finalize(
        ...
        self.tensor_useful_ops = _esum("tensor_useful_ops")
        self.tensor_padding_ops = _esum("tensor_padding_ops")
        self.tensor_energy_equivalent_ops = _esum("tensor_energy_equivalent_ops")
        if self.tensor_padding_ops > 0:
            self.tensor_padding_energy_activity = max(
                0.0,
                min(
                    1.0,
                    (self.tensor_energy_equivalent_ops - self.tensor_useful_ops)
                    / self.tensor_padding_ops,
                ),
            )
    ...
    def breakdown_by_name(self) -> list[dict]:
        ...
        groups: dict = defaultdict(lambda: {
            ...
            "tensor_useful_ops": 0.0,
            "tensor_padding_ops": 0.0,
            "tensor_energy_equivalent_ops": 0.0,
        })
        for n, s in self._entries:
            g = groups[s.op_name]
            g["n_total"]               += n
            ...
            g["tensor_useful_ops"]     += n * s.tensor_useful_ops
            g["tensor_padding_ops"]    += n * s.tensor_padding_ops
            g["tensor_energy_equivalent_ops"] += n * s.tensor_energy_equivalent_ops
        ...
        for op_name, g in groups.items():
            result.append({
                ...
                "tensor_useful_ops":     g["tensor_useful_ops"],
                "tensor_padding_ops":    g["tensor_padding_ops"],
                "tensor_energy_equivalent_ops": g["tensor_energy_equivalent_ops"],
            })
