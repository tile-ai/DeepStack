...
# Nominal one-operand heuristic motivated by arXiv:2409.18324,
# arXiv:2212.08805, and NVIDIA's CUTLASS matrix-fill methodology.  Prior work
# supports input-dependent switching power, not this exact 0.2 coefficient.
DEFAULT_TENSOR_PADDING_ENERGY_ACTIVITY = 0.8


def tensor_energy_equivalent_ops(
    useful_ops: float,
    padding_ops: float,
    empty_lane_activity: float = DEFAULT_TENSOR_PADDING_ENERGY_ACTIVITY,
) -> tuple[float, float]:
    """Apply total activity = 0.8 + 0.2*m/M to issued tensor work."""
    if not 0.0 <= empty_lane_activity <= 1.0:
        raise ValueError("empty_lane_activity must be in [0, 1]")
    issued_ops = useful_ops + padding_ops
    if issued_ops <= 0.0:
        return 0.0, 1.0
    useful_fraction = useful_ops / issued_ops
    total_activity = empty_lane_activity + (1.0 - empty_lane_activity) * useful_fraction
    return issued_ops * total_activity, total_activity
