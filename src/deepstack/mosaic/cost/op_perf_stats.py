...
class OpPerfStats:
    ...
    smem_bytes_per_tb: float = 0.0
    reg_bytes_per_thread: float = 0.0

    # ── Compute Utilization ──────────────────────────────────────────────────
    tensor_util_pct: float = 0.0
    cuda_util_pct: float = 0.0
    sfu_util_pct: float = 0.0
    tensor_useful_ops: float = 0.0
    tensor_padding_ops: float = 0.0
    tensor_energy_equivalent_ops: float = 0.0
    tensor_padding_energy_activity: float = 0.8
    ...
    noc_hop_time_s: float = 0.0
    noc_link_time_s: float = 0.0
    ...
    def _calc_chip_energy(
        ...
        *,
        ...
        """
        from mosaic.cost.energy import (
            DEFAULT_TENSOR_PADDING_ENERGY_ACTIVITY,
            EnergyModel,
            tensor_energy_equivalent_ops,
        )
        em = EnergyModel(
            arch,
            chip_energy_config=chip_energy_config,
        ...
        smem_io   = 0.0
        tensor_useful_ops = tensor_padding_ops = 0.0
        cuda_ops = sfu_ops = 0.0
        legacy_energy = (
            os.environ.get("DEEPSTACK_SWIGLU_COUNTS_MODE") == "paper_legacy"
        )
        ...
        for row, n in self._hete_rows:
            ...
            ddr_read_io  = float(row[7]) * n
            l2_read_io   = float(row[8]) * n
            t_util       = float(row[9])
            c_util       = float(row[10])
            s_util       = float(row[11])
            ...
            if not legacy_energy and len(row) >= 14:
                tensor_useful_ops += float(row[12]) * n
                tensor_padding_ops += float(row[13]) * n
            else:
                tensor_useful_ops += t_util * t_eff * arch.fp16_tensor_flops
            cuda_ops     += c_util * t_eff * arch.fp32_cuda_core_flops
            sfu_ops      += s_util * t_eff * arch.sfu_flops

        raw_activity = os.environ.get(
            "DEEPSTACK_TENSOR_PADDING_ENERGY_ACTIVITY",
            DEFAULT_TENSOR_PADDING_ENERGY_ACTIVITY,
        )
        padding_activity = float(raw_activity)
        if not 0.0 <= padding_activity <= 1.0:
            raise ValueError(
                "DEEPSTACK_TENSOR_PADDING_ENERGY_ACTIVITY must be in [0, 1], "
                f"got {padding_activity}"
            )
        tensor_energy_ops, tensor_total_activity = tensor_energy_equivalent_ops(
            tensor_useful_ops,
            tensor_padding_ops,
            padding_activity,
        )
        self.tensor_useful_ops = tensor_useful_ops
        self.tensor_padding_ops = tensor_padding_ops
        self.tensor_energy_equivalent_ops = tensor_energy_ops
        self.tensor_padding_energy_activity = padding_activity
        ...
        reg_io = smem_io + tensor_energy_ops * tc_regio_to_flops_ratio + cuda_ops * 2 * 4 + sfu_ops * 2 * 4
        ...
        tensor_e = em.get_tensor_core_energy(tensor_energy_ops)           * pj2j
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "tensor_work": {
                "useful_fp16_energy_equivalent_ops": self.tensor_useful_ops,
                "padding_fp16_energy_equivalent_ops": self.tensor_padding_ops,
                "padding_energy_activity": self.tensor_padding_energy_activity,
                "issued_tensor_activity": (
                    self.tensor_energy_equivalent_ops
                    / (self.tensor_useful_ops + self.tensor_padding_ops)
                    if self.tensor_useful_ops + self.tensor_padding_ops > 0
                    else 1.0
                ),
                "energy_equivalent_ops": self.tensor_energy_equivalent_ops,
            },
            "noc": {
