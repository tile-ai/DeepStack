...
def deepep_customized_mlp_coarse_by_token_stage1(shard_tokens:int, hidden:int, up_hidden:int, parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
...
def deepep_customized_mlp_coarse_by_token_stage2(shard_tokens:int, hidden:int, up_hidden:int, parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    assert granularity.get_mode() == "coarse"
    # print(parallel.world_size())
    ...
    element_mul_bytes=OpBytes(
        input1=Tensor_Loc(swiglu_bytes.input1.dtype, 'ddr',[shard_tokens, shard_up_hidden]),
...
def deepep_customized_mlp_coarse_by_token_stage3(shard_tokens:int, hidden:int, up_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    assert granularity.get_mode() == "coarse"
    # print(parallel.world_size())
...
def deepep_customized_mlp_coarse_by_token(shard_tokens:int, hidden:int, up_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    assert granularity.get_mode() == "coarse"
    # print(parallel.world_size())
    ...
    # W1: [shard_hidden, shard_up_hidden]
    # x@W1: [shard_bs, shard_seq, shard_up_hidden]
    # deepep_customized_mlp_coarse_by_token_stage1(bs:int, seq:int, hidden:int, up_hidden:int, parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    time_stage1, waves = deepep_customized_mlp_coarse_by_token_stage1(shard_tokens=shard_tokens, hidden=hidden, up_hidden=up_hidden, parallel=parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
    ...
    # ⊗ element-wise multiplication, (x @ W1) ⊗ silu(x @ W2) -> [bs/dp, seq/sp, up_hidden/tp], element-wise multiplication
    # time_stage2 =deepep_customized_mlp_coarse_by_token_stage2(shard_tokens=shard_tokens, hidden=hidden, up_hidden=up_hidden, parallel=parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
    time_stage2 = 0
    # ------------------------------------- stage 3 -------------------------------------
    # input[bs/dp, seq/sp, up_hidden/tp], W3[up_hidden/tp, hidden]
    # @ W3 -> [bs/dp, seq/sp, hidden]
    # tp-level all-reduce -> [bs/dp, seq/sp, hidden]
    time_stage3 =deepep_customized_mlp_coarse_by_token_stage3(shard_tokens=shard_tokens, hidden=hidden, up_hidden=up_hidden, parallel=parallel, next_parallel=next_parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)


    # ------------------------------------- stage 4 -------------------------------------
    # add residual
    # time_stage4 =deepep_customized_mlp_coarse_by_token_stage4(bs=bs, seq=seq, hidden=hidden, up_hidden=up_hidden, parallel=parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
