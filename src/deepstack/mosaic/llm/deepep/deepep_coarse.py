...
# from mosaic.llm.swiglu.deepep_customized_mlp_coarse_by_token import deepep_customized_mlp_coarse_by_token
from .deepep_customized_mlp_coarse_by_token import deepep_customized_mlp_coarse_by_token
...
def deepep_coarse_gate_shared_all_to_all(bs:int, seq:int, hidden:int, moe_down_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, expert_bytes:OpBytes, gate_bytes:OpBytes, num_shared_experts:int, num_routed_experts:int, num_activated_experts:int, routing_array:np.ndarray, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    ...
    def shared_experts():
        ...
        time_shared_experts, waves_shared_experts = deepep_customized_mlp_coarse_by_token(shard_tokens, hidden, moe_down_hidden, transformed_parallel, transformed_parallel, expert_bytes, granularity, single_chip, noc_hierarchy)
...
def deepep_coarse_routed_experts_all_to_all(bs:int, seq:int, hidden:int, moe_down_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, expert_bytes:OpBytes, gate_bytes:OpBytes, num_shared_experts:int, num_routed_experts:int, num_activated_experts:int, routing_array:np.ndarray, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    ...
    def routed_experts():     
        ...
        if (bs * seq >= 1024):
            ...
            if few_activated_times == 0:
                ...
                transformed_parallel = ParallelScheme(tp=parallel.tp, ep=1, sp=parallel.sp * parallel.ep2, cp=parallel.cp, dp=parallel.dp * parallel.ep1, fsdp=parallel.fsdp, pp=parallel.pp)
                time_few_activated_experts, waves_few_activated_experts = deepep_customized_mlp_coarse_by_token(shard_tokens, hidden, moe_down_hidden, transformed_parallel, transformed_parallel, expert_bytes, granularity, single_chip, noc_hierarchy)
            ...
            if more_activated_times == 0:
                ...
                transformed_parallel = ParallelScheme(tp=parallel.tp, ep=1, sp=parallel.sp * parallel.ep2, cp=parallel.cp, dp=parallel.dp * parallel.ep1, fsdp=parallel.fsdp, pp=parallel.pp)
                time_more_activated_experts, waves_more_activated_experts = deepep_customized_mlp_coarse_by_token(shard_tokens, hidden, moe_down_hidden, transformed_parallel, transformed_parallel, expert_bytes, granularity, single_chip, noc_hierarchy)
            ...
            for expert_count in counts:
                if (activated_tokens == 0):
                    ...
                    transformed_parallel = ParallelScheme(tp=parallel.tp, ep=1, sp=parallel.sp * parallel.ep2, cp=parallel.cp, dp=parallel.dp * parallel.ep1, fsdp=parallel.fsdp, pp=parallel.pp)
                    time_activated_experts, waves_activated_experts = deepep_customized_mlp_coarse_by_token(shard_tokens, hidden, moe_down_hidden, transformed_parallel, transformed_parallel, expert_bytes, granularity, single_chip, noc_hierarchy)
...
def deepep_coarse_token_weighted_summation(bs:int, seq:int, hidden:int, moe_down_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, expert_bytes:OpBytes, gate_bytes:OpBytes, num_shared_experts:int, num_routed_experts:int, num_activated_experts:int, routing_array:np.ndarray, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
...
def deepep_coarse(bs:int, seq:int, hidden:int, moe_down_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, expert_bytes:OpBytes, gate_bytes:OpBytes, num_shared_experts:int, num_routed_experts:int, num_activated_experts:int, routing_array:np.ndarray, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    ...
    time_gate_shared_all_to_all = deepep_coarse_gate_shared_all_to_all(bs, seq, hidden, moe_down_hidden, parallel, next_parallel, expert_bytes, gate_bytes, num_shared_experts, num_routed_experts, num_activated_experts, routing_array, granularity, single_chip, noc_hierarchy)
    
    time_routed_experts = deepep_coarse_routed_experts_all_to_all(bs, seq, hidden, moe_down_hidden, parallel, next_parallel, expert_bytes, gate_bytes, num_shared_experts, num_routed_experts, num_activated_experts, routing_array, granularity, single_chip, noc_hierarchy)

    time_token_weighted_summation = deepep_coarse_token_weighted_summation(bs, seq, hidden, moe_down_hidden, parallel, next_parallel, expert_bytes, gate_bytes, num_shared_experts, num_routed_experts, num_activated_experts, routing_array, granularity, single_chip, noc_hierarchy)
    ...
    time_deepep_coarse = time_gate_shared_all_to_all + time_routed_experts + time_token_weighted_summation
    log.info("time_deepep_coarse: %s", time_deepep_coarse)

    return  time_deepep_coarse
