...
def rope_top(bs:int, head:int, seq:int, head_dim:int, parallel:ParallelScheme, next_parallel:ParallelScheme, rope_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    ...
    if granularity.get_mode() == "coarse":
        return max_activation, mem_weight, rope_coarse(bs, head, seq, head_dim, parallel, next_parallel, rope_bytes, granularity, single_chip, noc_hierarchy)
