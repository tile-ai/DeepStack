...
def ropek_decoding_coarse_raw(bs:int, head:int, seq:int, head_dim:int, parallel:ParallelScheme, next_parallel:ParallelScheme, rope_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy, stats: "OpPerfStats | None" = None):
    ...
    if parallel.sp == next_parallel.cp and parallel.tp == next_parallel.tp:
        log.info("ropek_decoding_coarse_raw: parallel.sp == next_parallel.cp, no need to do all-gather")


        all_to_all_hop_latency = 0
        all_to_all_ext_max = 0
    elif parallel.sp == 1:
        log.info("ropek_decoding_coarse_raw: parallel.sp == 1, no need to do all-gather")
        all_to_all_hop_latency = 0
        all_to_all_ext_max = 0

    elif next_parallel.cp < parallel.sp and (parallel.cp*parallel.sp)==(next_parallel.cp*next_parallel.sp):
        log.info("ropek_decoding_coarse_raw: next_parallel.cp < parallel.sp, need to do all-gather for k rope")

        all_to_all_bytes = shard_bs * head_dim * math.ceil(head/parallel.tp) * (math.ceil(seq/next_parallel.cp) - shard_seq) * out_bytes  
        log.info("all_to_all_bytes: %s", all_to_all_bytes)
        
        new_virtual_parallel = ParallelScheme(tp=parallel.tp, ep=parallel.ep, sp=int(parallel.sp/next_parallel.cp), cp=1, dp=1, pp=int(parallel.world_size()/parallel.tp/parallel.ep/(parallel.sp/next_parallel.cp)))
        ...
        # api_latency, api_ext_max, api_traffic = all_gather_sp_fission(parallel=parallel, noc_hierarchy=noc_hierarchy, bytes_needed_per_device=all_to_all_bytes, next_virtual_sp=next_parallel.cp)
        ...
        raise NotImplementedError("ropek_decoding_coarse_raw: parallel :%s, next_parallel :%s not implemented", parallel, next_parallel)

    # return e2e_time

    return smem_fusion_list, all_to_all_hop_latency, all_to_all_ext_max
