...
def ropek_coarse(bs:int, head:int, seq:int, head_dim:int, parallel:ParallelScheme, next_parallel:ParallelScheme, rope_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy, stats: "OpPerfStats | None" = None):
    ...
    if parallel.sp == next_parallel.cp and parallel.tp == next_parallel.tp:
        log.info("ropek_coarse: parallel.sp == next_parallel.cp, no need to do all-gather")
        e2e_time = single_chip_time
    elif next_parallel.cp < parallel.sp and (parallel.cp*parallel.sp)==(next_parallel.cp*next_parallel.sp):
        log.info("ropek_coarse: next_parallel.cp < parallel.sp, need to do all-gather for k rope")
...
# )      
        ...
        raise NotImplementedError("ropek_coarse: parallel :%s, next_parallel :%s not implemented", parallel, next_parallel)
