...
from .gqa_decode_coarse import gqa_decode_coarse, gqa_decode_kv_list_coarse
...
def get_gqa_decode_footprint(bs:int, seq:int, cached_kv:int, hidden:int, num_head:int, num_kv_head:int, head_dim:int, parallel:ParallelScheme, atten_parallel:ParallelScheme, next_parallel:ParallelScheme, atten_bytes:OpBytes):
    ...
    k_act = in_bytes * shard_bs *shard_cached_kv * shard_hidden / group_size
    v_act = in_bytes * shard_bs *shard_cached_kv * shard_hidden / group_size
    ...
    # print("atten_parallel: %s", atten_parallel)
    # print("mem_kv_cache: %s GiB",  mem_kv_cache/(1024**3))
...
def gqa_decode_top(bs:int, seq:int, cached_kv:int, hidden:int, num_head:int, num_kv_head:int, head_dim:int, parallel:ParallelScheme, atten_parallel:ParallelScheme, next_parallel:ParallelScheme, atten_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):

    # assert parallel.ep==1

    # assert seq == 1
    # can i add some commets after assert
    # assert seq == 1, "seq must be 1 for gqa decode, as now we don't support mtp for gqa decode"
    assert parallel.ep==1
    if seq == 1:
        assert parallel.sp == 1

    # max_activation, mem_weight, mem_kv_cache = get_gqa_decode_footprint(bs, seq, cached_kv, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, atten_bytes)
    ...
    if granularity.get_mode() == "coarse":
        ...
        # return max_activation, mem_weight, mem_kv_cache, gqa_decode_coarse(bs, seq, cached_kv, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, atten_bytes, granularity, single_chip, noc_hierarchy)
        return gqa_decode_coarse(bs, seq, cached_kv, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, next_parallel, atten_bytes, granularity, single_chip, noc_hierarchy)
...
def gqa_decode_kv_list_top(bs:int, seq:int, cached_kv_list:list[int], hidden:int, num_head:int, num_kv_head:int, head_dim:int, parallel:ParallelScheme, atten_parallel:ParallelScheme, next_parallel:ParallelScheme, atten_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    ...
    if granularity.get_mode() == "coarse":
        ...
        # return max_activation, mem_weight, mem_kv_cache, gqa_decode_coarse(bs, seq, cached_kv, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, atten_bytes, granularity, single_chip, noc_hierarchy)
        return gqa_decode_kv_list_coarse(bs, seq, cached_kv_list, hidden, num_head, num_kv_head, head_dim, parallel, atten_parallel, next_parallel, atten_bytes, granularity, single_chip, noc_hierarchy)
