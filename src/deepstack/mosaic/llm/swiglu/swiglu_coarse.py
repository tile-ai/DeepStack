...
def swiglu_coarse_stage1(bs:int, seq:int, hidden:int, up_hidden:int, parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy, stats:"OpPerfStats | None" = None):
    ...
    hete_post_data, smem_fusion_post_data, tiling_config=gemm_wrapper(M=shard_bs*shard_seq, N=shard_up_hidden, K=hidden, gemm_bytes=gemm1_bytes, granularity=granularity, single_chip=single_chip)
...
def swiglu_coarse_stage2(bs:int, seq:int, hidden:int, up_hidden:int, parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy, stats:"OpPerfStats | None" = None):
    assert granularity.get_mode() == "coarse"
    # print(parallel.world_size())
    ...
    silu_bytes_stage1=OpBytes(
        input1=Tensor_Loc(swiglu_bytes.input1.dtype, 'smem',[shard_bs, shard_seq, shard_up_hidden]),
        input2=None,
        output=Tensor_Loc(swiglu_bytes.output.dtype, 'reg',[shard_bs, shard_seq, shard_up_hidden]),
    )
    ...
    silu_bytes_stage2=OpBytes(
        input1=Tensor_Loc(swiglu_bytes.input1.dtype, 'reg',[shard_bs, shard_seq, shard_up_hidden]),
        input2=None,
        output=Tensor_Loc(swiglu_bytes.output.dtype, 'smem',[shard_bs, shard_seq, shard_up_hidden]),
    )
    ...
    element_mul_bytes=OpBytes(
        input1=Tensor_Loc(swiglu_bytes.input1.dtype, 'ddr',[shard_bs, shard_seq, shard_up_hidden]),
        input2=Tensor_Loc(swiglu_bytes.input1.dtype, 'smem',[shard_bs, shard_seq, shard_up_hidden]),
        output=Tensor_Loc(swiglu_bytes.output.dtype, 'ddr',[shard_bs, shard_seq, shard_up_hidden]),
    )
...
def swiglu_coarse_stage3(bs:int, seq:int, hidden:int, up_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy, stats:"OpPerfStats | None" = None):
    assert granularity.get_mode() == "coarse"
    # print(parallel.world_size())
    ...
    # (M:int, N:int, K:int, gemm_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch)
    # happen to be no fusion, so this would be the final result, we will use the smem_fusion data only
    hete_post_data, smem_fusion_post_data, tiling_config=gemm_wrapper(M=shard_bs*shard_seq, N=hidden, K=shard_up_hidden, gemm_bytes=gemm3_bytes, granularity=granularity, single_chip=single_chip)
    # log.debug("smem_fusion_post_data: %s, tiling_config: %s", smem_fusion_post_data, tiling_config)
...
def swiglu_coarse_stage4(bs:int, seq:int, hidden:int, up_hidden:int, parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy, stats:"OpPerfStats | None" = None):
    assert granularity.get_mode() == "coarse"
    # print(parallel.world_size())
    ...
    # (x @ W1) ⊗ silu(x @ W2) -> [bs/dp, seq/sp, up_hidden/tp]
    # @ W3 -> [bs/dp, seq/sp, hidden]
    # add residual [bs/dp, seq/sp, hidden] + [bs/dp, seq/sp, hidden] -> [bs/dp, seq/sp, hidden]
    ...
    silu_bytes_stage2=OpBytes(
        input1=Tensor_Loc(swiglu_bytes.output.dtype, 'ddr',[shard_bs, shard_seq, hidden]),
        input2=Tensor_Loc(swiglu_bytes.output.dtype, 'ddr',[shard_bs, shard_seq, hidden]),
        output=Tensor_Loc(swiglu_bytes.output.dtype, 'ddr',[shard_bs, shard_seq, hidden]),
    )

    hete_post_data_residual, tb_residual_config=element_wrapper(element_op_bytes=silu_bytes_stage2, granularity=granularity, single_chip=single_chip, batch=2, type="cuda_core", tb_tiling_config=None)
    smem_fusion_list.append(hete_reg_fusion([hete_post_data_residual], single_chip))

    # ------------------------------------- stage 2.3 ⊗  (x @ W1) ⊗ silu(x @ W2) -> [bs/dp, seq/sp, up_hidden/tp] smem -------------------------------------

    grids = [shard_bs/tb_residual_config[0], shard_seq/tb_residual_config[1], hidden/tb_residual_config[2]]
    ...
    log.info("swiglu stage 4 (Residual Add) single chip time: %s s", single_chip_time)
...
def swiglu_coarse(bs:int, seq:int, hidden:int, up_hidden:int, parallel:ParallelScheme, next_parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    assert granularity.get_mode() == "coarse"
    # print(parallel.world_size())

    stats = OpPerfStats(op_name="swiglu", dump_perf_log=granularity.dump_perf_log) if granularity.dump_perf_log else None
    ...
    # W1: [shard_hidden, shard_up_hidden]
    # x@W1: [shard_bs, shard_seq, shard_up_hidden]
    # swiglu_coarse_stage1(bs:int, seq:int, hidden:int, up_hidden:int, parallel:ParallelScheme, swiglu_bytes:OpBytes, granularity:Modeling_Granularity, single_chip:Arch, noc_hierarchy:Hierarchy):
    time_stage1 = swiglu_coarse_stage1(bs=bs, seq=seq, hidden=hidden, up_hidden=up_hidden, parallel=parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy, stats=stats)
    ...
    # ⊗ element-wise multiplication, (x @ W1) ⊗ silu(x @ W2) -> [bs/dp, seq/sp, up_hidden/tp], element-wise multiplication
    time_stage2 =swiglu_coarse_stage2(bs=bs, seq=seq, hidden=hidden, up_hidden=up_hidden, parallel=parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy, stats=stats)

    # ------------------------------------- stage 3 -------------------------------------
    # input[bs/dp, seq/sp, up_hidden/tp], W3[up_hidden/tp, hidden]
    # @ W3 -> [bs/dp, seq/sp, hidden]
    # tp-level all-reduce -> [bs/dp, seq/sp, hidden]
    time_stage3 =swiglu_coarse_stage3(bs=bs, seq=seq, hidden=hidden, up_hidden=up_hidden, parallel=parallel, next_parallel=next_parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy, stats=stats)


    # ------------------------------------- stage 4 -------------------------------------
    # add residual
    # time_stage4 =swiglu_coarse_stage4(bs=bs, seq=seq, hidden=hidden, up_hidden=up_hidden, parallel=parallel, swiglu_bytes=swiglu_bytes, granularity=granularity, single_chip=single_chip, noc_hierarchy=noc_hierarchy)
    ...
    log.info("Whole swiglu coarse e2e time: %s s", total_time)
    if stats is not None:
