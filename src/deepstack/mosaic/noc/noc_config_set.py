...
Most named reference profiles are binary-backed; the H200 example is defined
below. Fully caller-defined topologies
...
def h200x32(nvlink_bw=370e9, ib_bw=50e9):
    """Four nodes, eight H200 GPUs each. Bandwidth arguments are bytes/s."""
    if not all(math.isfinite(value) and value > 0 for value in (nvlink_bw, ib_bw)):
        raise ValueError("NVLink and InfiniBand bandwidth must be finite and positive")
    # L1: the eight GPUs inside one node, connected through NVSwitch.
    L1 = make_switch(
        8, hop_latency=2.5 / 2 / 1e6, link_bandwidth=nvlink_bw,
        switch_center_in_bw=nvlink_bw * 8, switch_center_out_bw=nvlink_bw * 8,
    )
    # L2: four nodes connected through InfiniBand.
    L2 = make_switch(
        4, hop_latency=5 / 1e6, link_bandwidth=ib_bw,
        switch_center_in_bw=ib_bw * 4, switch_center_out_bw=ib_bw * 4,
    )
    # L3: the unused outer level required by the three-level hierarchy.
    L3 = make_switch(
        1, hop_latency=0, link_bandwidth=ib_bw * 10,
        switch_center_in_bw=ib_bw * 10 * 8, switch_center_out_bw=ib_bw * 10 * 8,
    )
    return Hierarchy(
        layers=[L3, L2, L1], port_spread=PortSpread.EVEN,
        node_mapper=None, name="h200x32",
    )
