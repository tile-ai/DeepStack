...
# --- 1. Core routing simulation function ---
def simulate_token_routing(
    token_embedding: torch.Tensor,
    router_group_net: nn.Module,  # No longer used for scoring, but retain the parameter for compatibility
    ...
    # Retrieve the recorded k_per_group expert indices from each selected group
    selected_experts = []
    for g in selected_groups_list:
        selected_experts.extend(group_top_indices[g])

    # Sanity check: the count should be exactly num_experts_per_tok
...
def run_simulation(config: dict):
    ...
    embedding_dim = 2048
    router_group_net = nn.Linear(embedding_dim, config["n_group"])            # Retained for compatibility
    router_expert_net = nn.Linear(embedding_dim, config["n_routed_experts"])  # Main scoring network
...
# Convert routing_results to a 2D list, with each sublist containing the expert IDs for one token
...
def import_results_as_expert_id_lists(filepath: str, n: Optional[int] = None) -> list:
    ...
    # Try streaming parsing first to avoid loading the entire JSON
    try:
        import ijson  # Optional dependency: pip install ijson

        expert_id_lists = []
        with open(filepath, 'r', encoding='utf-8') as f:
            # Iterate over entries under routing_results one key/value pair at a time
        ...
        # Raise an error if the stream ends before n entries are read
        raise ValueError(
            f"请求的前 {n} 个 routed list，但实际只有 {len(expert_id_lists)} 个"
        )
    except ImportError:
        # Fallback: read everything at once (still construct only the first n entries)
        loaded = import_results(filepath)
        routing_results = loaded.get("routing_results", {})
        ...
        # Fall back to the safe path on any streaming exception
        loaded = import_results(filepath)
        routing_results = loaded.get("routing_results", {})
        expert_id_lists = extract_selected_experts_lists(routing_results, n=n)
        if len(expert_id_lists) < n:
...
# --- 3.1 Read from NPZ and flatten to [:, expected_last_dim] ---
def load_npz_routing_flatten_last_dim(npz_path: str, as_list: bool = False, expected_last_dim: int = 8):
    ...
    def _reshape_to_rows(arr: np.ndarray, name: str) -> np.ndarray:
        ...
        # For object arrays, first try converting the whole array to numeric (suitable for arrays of scalar objects)
        if arr.dtype == object:
            try:
                ...
                # If the last dimension is not 8, try flattening to 1D and regrouping
                ...
                # Fallback: collect elements individually, then group them into sets of 8
        ...
        # Regular numeric arrays: first try processing with a last dimension of 8
        if arr.ndim >= 1 and arr.shape[-1] == expected_last_dim:
            return arr.reshape(-1, expected_last_dim)
        # Fallback: flatten to 1D, then regroup
...
# --- 3.2 Read from NPZ (preserving the original shape) and assert the last dimension is expected_last_dim ---
def load_npz_routing_keep_shape(npz_path: str, as_list: bool = False, expected_last_dim: int = 8):
    ...
    def _to_numeric_keep_shape(arr: np.ndarray, name: str) -> np.ndarray:
        if arr is None:
            return None
        if arr.dtype == object:
            # Convert the whole array to numeric (suitable for object arrays containing scalars)
...
def example_gen_routing_list():
    ...
    simulation_config = {
        ...
        "n_group": 8,
        "topk_group": 4,            # How many groups to select
        "num_experts_per_tok": 8,   # Total number of experts to select (must be divisible by topk_group)
        # Change 1: default output directory is ../data/routing_outputs
    ...
    if os.path.exists(filepath):
        ...
        # Example: convert to a 2D list and display the first two tokens
        ...
        # Example: convert in-memory results directly to a 2D list and display the first two tokens
...
def extract_prefill_and_decode_from_npz_as_list():
    ...
    # Extract the substring after _a and before the last / in npz_path
    # For example: extract 128 from "../data/qwen_e128_a128/moe_activations.npz"
    ...
    # Find the smallest number in decode_list
    # min_value = decode_list.min()
    # print("Minimum value in decode_list:", min_value)
    # max_value = decode_list.max()
    # print("Maximum value in decode_list:", max_value)
    return prefill_list, decode_list
