...
class A100(Arch):
    ...
    def set_to_microbench(self):
        # GPU frequency measured via nop: 1409.21 MHz, via fmac: 1386.87 MHz
        self.core_freq = 1.41 * 1e9
        self.base_freq = self.core_freq
        self.max_freq = self.core_freq

        # DRAM bandwidth: read 1654.81 GB/s, write 1692.90 GB/s, copy 1472.93 GB/s
        self.ddr_bandwidth = 1654.812012 * 1e9  # use read bandwidth

        # L2 cache bandwidth: 3234.77 GB/s
        self.l2_bandwidth = 3234.773083 * 1e9

        # Shared memory: measured 111.76 byte/cycle per SM, theoretical 128
        self.l1_smem_throughput_per_cycle = 111.758698
        self.smem_bandwidth = 19491.839844 * 1e9  # whole chip measured

        # FP16 tensor core: measured 298.951 TFLOPS (theoretical 311.87)
        self.fp16_tensor_flops = 298.951 * 1e12

        # FP32 CUDA core: measured 19.017 TFLOPS (theoretical 19.49)
